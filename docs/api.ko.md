...
| 인증 | 기본적으로 없음. `TUBEDEPTH_REQUIRE_API_KEY`를 켠 배포에서만 `/v1` 아래 전부에 `X-API-Key: ytd_...` |
...
바인딩하면 그 키가 평문으로 회선에 실린다 — 그리고 인증이 꺼져 있으면 아무나 잡을 등록할
수 있는 API를 회선에 올리는 것이 된다.
...
**기본값은 꺼짐이다.** 이 서비스는 사설망에 배포되어 플릿의 다른 서비스가 호출한다. 거기서
호출자마다 키를 발급하는 것은 감사 컬럼과 레이트 리밋을 얻는 대신 배포하고 교체해야 할
비밀을 하나 늘리는 일이었다. 그래서 `/v1`은 헤더를 받지 않고, 401을 내지 않으며, 키별
할당량도 적용하지 않는다 — 이 배포가 그렇게 요구하지 않는 한.

```sh
TUBEDEPTH_REQUIRE_API_KEY=1     # deploy/.env, 또는 유닛의 EnvironmentFile
```

시작할 때 한 번 읽으므로 값을 바꾸려면 재시작해야 한다. 예(`1`, `true`, `yes`, `on`)도
아니오(`0`, `false`, `no`, `off`)도 아닌 값은 아니오로 읽지 않고 시작 시점에 거부한다 —
`=treu`가 조용히 열린 API를 서빙하는 것이 이 변수가 존재하는 이유인 실패다.

아래는 전부 **켜져 있을 때**를 설명한다. **꺼져 있으면 이 문서의 `401 unauthenticated`는
일어나지 않고 `X-API-Key`는 선택이다 — 다만 무시되지는 않는다. 보낸 키는 여전히 검증되므로
호출자는 자기 귀속 이력과 할당량을 유지하고, 폐기된 키는 조용히 익명 접근으로 승격되는 대신
그대로 실패한다.**
...
| 상태 | 코드 | 뜻 |
| --- | --- | --- |
| 401 | `unauthenticated` | 키가 없거나, 형식이 틀렸거나, 모르는 키이거나, 폐기됨. 키를 요구하지 않는 배포(인증 절 참고)에서는 키를 보냈는데 검증에 실패한 경우 외에는 나오지 않는다 |
