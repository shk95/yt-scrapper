...
| 4 connection budget | **적용됨(#15, Task 7-8; 전체 브랜치 리뷰로 2026-08-20 재조정).** manifest에 상한 32 선언(이전 20에서 상향); `tubedepth_runtime`에 `CONNECTION LIMIT 32`. `Database`의 write engine은 이제 `--concurrency`로 크기가 정해진다 — API는 여전히 `pool_size=2, max_overflow=2`(engine당 4)이지만, worker는 `pool_size=max_overflow=TUBEDEPTH_CONCURRENCY`(기본 8이면 engine당 16)로 연다. `Worker.drain`이 concurrency당 claim thread 하나와 lease 갱신 thread 하나를 돌리고 둘 다 write engine에서 세션을 얻으므로, concurrency 8이면 순간적으로 최대 16개 세션이 4-연결 pool을 다툴 수 있다는 것을 실측으로 확인(스레드 수로 추정한 것이 아니라, 16개 동시 세션 요청을 pool_size=2/max_overflow=2에 걸어 측정 — 4개씩 배치로 직렬화되고 뒤 배치일수록 앞 배치의 세션 점유 시간만큼 대기가 쌓임을 확인). read engine은 `Worker.reap`에서만 쓰여 동시성 압박이 없으므로 기본값(4) 유지. steady state: API 8 + worker(16+4=20) = 28, +migration 1, +rolling-overlap 0 = 29 ≤ 32, 여유 3(상향 전과 동일한 여유폭) | `src/tubedepth/database.py`, `src/tubedepth/cli.py`, `deploy/service-manifest.yaml`, `deploy/postgres-bootstrap.sql` |
