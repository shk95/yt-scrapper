...
| 4 connection budget | **적용됨(#15, Task 7-8; 전체 브랜치 리뷰로 2026-08-20 재조정, 그리고 조정 자체를 다시 조정).** manifest 상한은 여전히 20, `tubedepth_runtime`도 여전히 `CONNECTION LIMIT 20` — 처음엔 32로 올렸다가 되돌렸다. 규정 4는 connection budget을 **database 전체**의 예산으로 다루라고 하고, 20은 이 서비스가 함대에 *요청해서 받은* 몫이다. 자기 pool 산수가 부족하다고 자기 선언을 스스로 올리는 것은 문제를 푸는 게 아니라, 인스턴스가 바닥났을 때 걸리는 다른 서비스에게 실패를 떠넘기는 것이다. `Database`의 write engine은 이제 `--concurrency`로 크기가 정해진다(`pool_size=max_overflow=TUBEDEPTH_CONCURRENCY`) — 이 자체는 옳다: `Worker.drain`이 concurrency당 claim thread 하나와 lease 갱신 thread 하나를 돌리고 둘 다 write engine에서 세션을 얻으므로, concurrency 8이면 순간적으로 최대 16개 세션이 4-연결 pool을 다툴 수 있다는 것을 실측으로 확인(스레드 수로 추정한 것이 아니라, 16개 동시 세션 요청을 pool_size=2/max_overflow=2에 걸어 측정 — 4개씩 배치로 직렬화되고 뒤 배치일수록 앞 배치의 세션 점유 시간만큼 대기가 쌓임을 확인). 문제는 이 발견 뒤에 예산을 20에서 32로 올려서 맞춘 것 — 되돌리고, 대신 20 안에 들어가도록 `TUBEDEPTH_CONCURRENCY`를 낮췄다: API 8(write 4 + read 4) + worker 8(write 2×2=4 + read 4, read는 `Worker.reap`만 쓰므로 concurrency와 무관하게 기본값 유지) = 16, +migration 1, +rolling-overlap 0 = 17 ≤ 20, 여유 3 — concurrency 도입 전과 정확히 같은 총합과 여유폭. 배포 기본값(`deploy/tubedepth-worker.service`)을 8에서 2로 내렸다. **미해결로 남기는 것**: AIMD 컨트롤러가 실측한 유효 상한은 6(위 §를 보라 — 6을 넘으면 병목이 우리 쪽이 아니다)인데, 6은 20 예산 안에 들어가지 않는다(2×6+4=16짜리 worker write만으로도 API 8과 합쳐 24, +margin으로 더). 이것은 함대에 예산 증액을 **요청할** 사안이지, 이 저장소가 혼자 결정할 사안이 아니다 — 요청 자체는 아직 열려 있다 | `src/tubedepth/database.py`, `src/tubedepth/cli.py`, `deploy/service-manifest.yaml`, `deploy/postgres-bootstrap.sql`, `deploy/tubedepth-worker.service` |
