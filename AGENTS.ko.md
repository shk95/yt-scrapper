...
description이 왜 그 일이 존재하고 어디서 시작하는지를 말하고, 이슈 본문이 무엇을 해야 하는지를
말한다. 마일스톤에 없는 열린 이슈는 순서가 없는 단독 작업이다. 클론이 아니라 GitHub로 들어온
사람을 위한 같은 지도는 고정된 **"Start here"** 이슈다.
...
- **`ExtractionError`(파서 문제)는 egress 건강을 절대 건드리지 않는다.** YouTube가 렌더러 이름을 바꾼 날
  ...
  `NEUTRAL`이다 — 모르는 실패가 멀쩡한 주소를 태우게 두지 않는다. `decisions/001` 참조.
...
문서는 독자에 따라 갈린다. **저장소 바깥 사람이 읽는 넷 — `README.md`, `docs/api.md`,
`CHANGELOG.md`, 그리고 이 파일 — 은 영어가 정본이고, 옆의 `.ko.md`가 번역이다. 둘 다 고친다.**
...
그 밖의 문서(status·troubleshooting·definition-of-done·releasing·plan)는 기여자용이므로
...
**모든 커밋은 그 자체로 `just check` 초록인 단위여야 한다** — 다음 커밋이 있어야만 동작하는 커밋은
bisect로 지나갈 수 없는 커밋이다.
